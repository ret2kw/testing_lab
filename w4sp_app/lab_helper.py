...
def runshark(name):
    ...
    if name == 'root':
        p1 = Process(target=r, args=('wireshark',))
        p2 = Process(target=r, args=('xterm -fn 10x20',))
        p1.start()
        p2.start()
        return 

    #start wireshark in a different process
    wireshark = 'ip netns exec %s wireshark' % name
    p = Process(target=r, args=(wireshark,))
    p.start()

    #start terminal in another process
    terminal = 'ip netns exec %s xterm -fn 10x20' % name
    p = Process(target=r, args=(terminal,))
    p.start()
